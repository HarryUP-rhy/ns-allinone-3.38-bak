...
PhyEntity::SnrPer
PhyEntity::GetPhyHeaderSnrPer(WifiPpduField field, Ptr<Eventt> event) const
{
...
void
PhyEntity::StartReceiveField(WifiPpduField field, Ptr<Eventt> event)
{
...
void
PhyEntity::EndReceiveField(WifiPpduField field, Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << field << *event);
    NS_ASSERT(m_wifiPhy); // no sense if no owner WifiPhy instance
    NS_ASSERT(m_wifiPhy->m_endPhyRxEvent.IsExpired());
...
bool
PhyEntity::DoStartReceiveField(WifiPpduField field, Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << field << *event);
...
PhyEntity::DoEndReceiveField(WifiPpduField field, Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << field << *event);
                                ...
                                Time rxDuration)
{
    ...
    Ptr<Eventt> event = m_wifiPhy->GetPhyEntityForPpdu(ppdu)->DoGetEvent(
        ppdu,
...
void
PhyEntity::StartReceivePayload(Ptr<Eventt> event)
{
...
Time
PhyEntity::DoStartReceivePayload(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
    Ptr<const WifiPpdu> ppdu = event->GetPpdu();
...
void
PhyEntity::ScheduleEndOfMpdus(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
    Ptr<const WifiPpdu> ppdu = event->GetPpdu();
...
void
PhyEntity::EndOfMpdu(Ptr<Eventt> event,
...
void
PhyEntity::EndReceivePayload(Ptr<Eventt> event)
{
    Ptr<const WifiPpdu> ppdu = event->GetPpdu();
    WifiTxVector txVector = event->GetTxVector();
    Time psduDuration = ppdu->GetTxDuration() - CalculatePhyPreambleAndHeaderDuration(txVector);
...
const std::map<std::pair<uint64_t, WifiPreamble>, Ptr<Eventt>>&
...
void
PhyEntity::AddPreambleEvent(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
    Ptr<const WifiPpdu> ppdu = event->GetPpdu();
...
void
PhyEntity::UpdateInterferenceEvent(Ptr<Eventt> event, const RxPowerWattPerChannelBand& rxPower)
{
...
PhyEntity::PhyFieldRxStatus
PhyEntity::DoEndReceivePreamble(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
...
void
PhyEntity::StartPreambleDetectionPeriod(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
...
void
PhyEntity::EndPreambleDetectionPeriod(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
...
void
PhyEntity::ResetReceive(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
...
void
PhyEntity::DoResetReceive(Ptr<Eventt> event)
{
    NS_LOG_FUNCTION(this << *event);
    NS_ASSERT(event->GetEndTime() == Simulator::Now());
}
...
double
PhyEntity::GetRxPowerWForPpdu(Ptr<Eventt> event) const
{
