 ...
 */
class Eventt : public SimpleRefCount<Eventt>
{
  public:
    /**
     ...
     */
    Eventt(Ptr<const WifiPpdu> ppdu,
 ...
 */
std::ostream& operator<<(std::ostream& os, const Eventt& event);
...
class InterferenceHelper : public Object
{
  public:
     ...
     * \return Event
     */
    Ptr<Eventt> Add(Ptr<const WifiPpdu> ppdu,
                   const WifiTxVector& txVector,
                   Time duration,
                   RxPowerWattPerChannelBand& rxPower,
     ...
     */
    double CalculateSnr(Ptr<Eventt> event,
                        uint16_t channelWidth,
                        uint8_t nss,
     ...
     */
    struct PhyEntity::SnrPer CalculatePhyHeaderSnrPer(Ptr<Eventt> event,
                                                      uint16_t channelWidth,
                                                      WifiSpectrumBand band,
     ...
     */
    void UpdateEvent(Ptr<Eventt> event, const RxPowerWattPerChannelBand& rxPower);
  ...
  private:
    ...
    class NiChange
    {
      public:
         ...
         */
        NiChange(double power, Ptr<Eventt> event);
        ~NiChange();
        /**
         * Return the power
         *
         ...
         * \return the event
         */
        Ptr<Eventt> GetEvent() const;

      private:
        double m_power;     ///< power in watts
        Ptr<Eventt> m_event; ///< event
    };
     ...
     */
    void AppendEvent(Ptr<Eventt> event, bool isStartOfdmaRxing);
     ...
     */
    double CalculateNoiseInterferenceW(Ptr<Eventt> event,
     ...
     */
    double CalculatePayloadPer(Ptr<const Eventt> event,
                               uint16_t channelWidth,
                               NiChangesPerBand* nis,
                               WifiSpectrumBand band,
                               uint16_t staId,
     ...
     */
    double CalculatePhyHeaderPer(Ptr<const Eventt> event,
                                 NiChangesPerBand* nis,
                                 uint16_t channelWidth,
                                 WifiSpectrumBand band,
                                 WifiPpduField header) const;
     ...
     */
    double CalculatePhyHeaderSectionPsr(Ptr<const Eventt> event,
                                        NiChangesPerBand* nis,
                                        uint16_t channelWidth,
                                        WifiSpectrumBand band,
