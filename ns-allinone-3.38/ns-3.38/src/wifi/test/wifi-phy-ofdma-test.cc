...
class OfdmaSpectrumWifiPhy : public SpectrumWifiPhy
{
  public:
     ...
     */
    std::map<std::pair<uint64_t, WifiPreamble>, Ptr<Eventt>>& GetCurrentPreambleEvents();
    /**
     * \return the current event
     */
    Ptr<Eventt> GetCurrentEvent();
...
std::map<std::pair<uint64_t, WifiPreamble>, Ptr<Eventt>>&
...
TestUlOfdmaPhyTransmission::CheckNonOfdmaRxPower(Ptr<OfdmaSpectrumWifiPhy> phy,
                                                 ...
                                                 double expectedRxPower)
{
    Ptr<Eventt> event = phy->GetCurrentEvent();
    NS_ASSERT(event);
