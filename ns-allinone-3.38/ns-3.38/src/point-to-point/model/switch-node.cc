...
SwitchNode::SwitchNode(){
	m_ecmpSeed = GetId();//m_ecmpSeed = m_id;
	m_node_type = 1;
...
void SwitchNode::SendToDev(Ptr<Packet>p, CustomHeader &ch){
	...
	if (idx >= 0){
		...
		GetDevice(idx)->SwitchSend(qIndex, p, ch);
	}else
		return; // Drop
}
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p){
	...
	if (1){
		// std::cout << "=== Packet Buffer Analysis ===" << '\n';
		// std::cout << "Packet size: " << p->GetSize() << '\n';
		// std::cout << "Packet UID: " << p->GetUid() << '\n';
		uint8_t* buf = p->GetBuffer();
		// std::cout << "GetBuffer() returned: " << (void*)buf << '\n';
		if (buf[PppHeader::GetStaticSize() + 9] == 0x11){ // udp packet
			/*rhy add start*/
			uint32_t seqTsSize = SeqTsHeader().GetSerializedSize();
			uint32_t totalHeaderSize = PppHeader::GetStaticSize() + 20 + 8 + seqTsSize;
			// std::cout <<"PushHop: offset=" << totalHeaderSize << ", pktSize=" << p->GetSize()<<'\n';
			// 关键修复：确保我们有足够的空间
			uint32_t intHeaderSize = sizeof(IntHeader);
			if (p->GetSize() < totalHeaderSize + intHeaderSize) {
				// std::cout <<"Packet too small for INT header. Needed: " 
				// // 		<< totalHeaderSize + intHeaderSize 
						// << ", Available: " << p->GetSize() << '\n';
				return;
			}
			/*rhy add end*/
			// 关键修复：正确的指针计算
			// 确保 IntHeader 在分配的缓冲区范围内
			uint8_t* bufferStart = p->GetBuffer();
			uint32_t bufferSize = p->GetSize();
			
			// 计算安全的 IntHeader 位置
			uint32_t intHeaderOffset = totalHeaderSize;
			uint8_t* intHeaderPtr = bufferStart + intHeaderOffset;
			
			// std::cout << "=== Memory Safety Check ===" << '\n';
			// std::cout << "Buffer range: " << (void*)bufferStart << " - " 
			// 		<< (void*)(bufferStart + bufferSize) << '\n';
			// std::cout << "IntHeader desired at: " << (void*)intHeaderPtr << '\n';
			// std::cout << "IntHeader ends at: " << (void*)(intHeaderPtr + intHeaderSize) << '\n';
			// 安全检查：确保 IntHeader 完全在包缓冲区内
			if (intHeaderPtr < bufferStart || 
				(intHeaderPtr + intHeaderSize) > (bufferStart + bufferSize)) {
				// std::cout << "ERROR: IntHeader would be outside packet buffer!" << '\n';
				return;
			}
			// 使用 reinterpret_cast 更安全
        	IntHeader* ih = reinterpret_cast<IntHeader*>(intHeaderPtr);
			//IntHeader *ih = (IntHeader*)&buf[PppHeader::GetStaticSize() + 20 + 8 + 6]; // ppp, ip, udp, SeqTs, INT	rhyadd
			Ptr<QbbNetDevice> dev = DynamicCast<QbbNetDevice>(GetDevice(ifIndex));
			if (m_ccMode == 3){ // HPCC
				//std::cout << "===rhy test=== 1111\t"<<'\n';
				//std::cout << "===rhy test=== \t"<<"ih->maxhop"<<"\t" <<ih->maxHop<<'\n';
				//ih->nhop = (ih->nhop < IntHeader::maxHop) ? ih->nhop : 0;
				ih->nhop = 0;	//后续修改跳数信息
