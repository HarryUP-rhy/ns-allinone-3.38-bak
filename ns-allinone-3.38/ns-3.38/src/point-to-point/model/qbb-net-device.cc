...
namespace ns3 {
	...
	Ptr<Packet> RdmaEgressQueue::DequeueQindex(int qIndex){
		...
		//std::cout << "===rhy test=== 4444-5-1\t"<<'\n';
		if (qIndex >= 0){ // qp
			Ptr<Packet> p = m_rdmaGetNxtPkt(m_qpGrp->Get(qIndex));
			//std::cout << "===rhy test=== 4444-5-2\t"<<'\n';
			m_rrlast = qIndex;
			m_qlast = qIndex;
	...
	int RdmaEgressQueue::GetNextQindex(bool paused[]){
		...
		for (qIndex = 1; qIndex <= fcount; qIndex++){
			...
			if (!paused[qp->m_pg] && qp->GetBytesLeft() > 0 && !qp->IsWinBound()){
				if (m_qpGrp->Get(idx)->m_nextAvail.GetTimeStep() > Simulator::Now().GetTimeStep()) //not available now
					continue;
				res = static_cast<int>(idx);//显式类型转换
				break;
		...
		if (min_finish_id < 0xffffffff){
			uint32_t nxt = min_finish_id;//nxt原来为int类型
			auto &qps = m_qpGrp->m_qps;
			for (uint32_t i = min_finish_id + 1; i < fcount; i++) if (!qps[i]->IsFinished()){
				if (i == static_cast<uint32_t>(res)) // update res to the idx after removing finished qp 显式类型转换
					res = static_cast<int>(nxt);//显式类型转换
				qps[nxt] = qps[i];
				nxt++;
			}
			qps.resize(nxt);
	...
	{
		//std::cout << "===rhy test=== 4444\t"<<'\n';
		NS_LOG_FUNCTION(this);
		if (!m_linkUp) return; // if link is down, return
		if (m_txMachineState == BUSY) return;	// Quit if channel busy
		Ptr<Packet> p;
		//std::cout << "===rhy test=== 4444-1\t"<<'\n';
		if (m_node->GetNodeType() == 0){
			//std::cout << "===rhy test=== 4444-2\t"<<'\n';
			int qIndex = m_rdmaEQ->GetNextQindex(m_paused);
			if (qIndex != -1024){
				std::cout << "[QBB][NIC] node=" << m_node->GetId()
					<< " if=" << m_ifIndex
					<< " qIndex=" << qIndex
					<< " paused=" << m_paused[(qIndex == -1)?0:qIndex]
					<< std::endl;
				//std::cout << "===rhy test=== 4444-3\t qIndex = "<<qIndex<<'\n';
				if (qIndex == -1){ // high prio
					p = m_rdmaEQ->DequeueQindex(qIndex);
					m_traceDequeue(p, 0);
					std::cout << "[QBB][NIC] node=" << m_node->GetId()
						<< " high-prio dequeue size=" << (p? p->GetSize():0)
						<< std::endl;
					TransmitStart(p);
					return;
				}
				//std::cout << "===rhy test=== 4444-4\t"<<'\n';
				// a qp dequeue a packet
				Ptr<RdmaQueuePair> lastQp = m_rdmaEQ->GetQp(qIndex);
				//std::cout << "===rhy test=== 4444-5\t"<<'\n';
				p = m_rdmaEQ->DequeueQindex(qIndex);
				std::cout << "[QBB][NIC] node=" << m_node->GetId()
					<< " qp dequeue size=" << (p? p->GetSize():0)
					<< " snd_nxt=" << (lastQp? lastQp->snd_nxt:0)
					<< " snd_una=" << (lastQp? lastQp->snd_una:0)
					<< std::endl;
				//std::cout << "===rhy test=== 4444-6\t p->GetSize() = "<<p->GetSize()<<'\n';
				// transmit
				m_traceQpDequeue(p, lastQp);
				TransmitStart(p);
				//std::cout << "===rhy test=== 4444-7\t"<<'\n';
			...
			// std::cout << "===rhy test=== 3333\t node type = "<<m_node->GetNodeType()<<'\n';
			p = m_queue->DequeueRR(m_paused);		//this is round-robin
			// std::cout << "===rhy test=== 3333-1\t p-> "<<(void*)p<<'\n';
			//std::cout << "===rhy test=== 3333-2\t p->GetBuffer() = "<<(void*)p->GetBuffer()<<'\n';
			if (p != nullptr){
				m_snifferTrace(p);
				...
				// std::cout << "===rhy test=== 3333-3\t qIndex =  "<<qIndex<<'\n';
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		std::cout << "[QBB][RECV] node=" << m_node->GetId()
			<< " if=" << m_ifIndex
			<< " size=" << (packet? packet->GetSize() : 0)
			<< std::endl;
		if (!m_linkUp){
		...
		if (ch.l3Prot == 0xFE){ // PFC
			...
			if (m_node->GetNodeType() > 0){ // switch
				std::cout << "[QBB][SWITCH-RECV] node=" << m_node->GetId()
					<< " ingressIf=" << m_ifIndex
					<< " forward" << std::endl;
			...
			// }
				...
				std::cout << "[QBB][HOST-RECV] node=" << m_node->GetId()
					<< " delivering to RDMA" << std::endl;
				// Trace callback state before handing the packet to RDMA.
				bool rdmaCbMissing = m_rdmaReceiveCb.IsNull();
				std::cout << "[QBB][HOST-RECV] node=" << m_node->GetId()
					<< " rdmaCbNull=" << (rdmaCbMissing ? "yes" : "no")
					<< std::endl;
				if (rdmaCbMissing){
					return;
				}
