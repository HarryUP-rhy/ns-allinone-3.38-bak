...
//#include "../../mpi/model/point-to-point-remote-channel.h"
#include "ns3/abort.h"
#include "ns3/log.h"
...
QbbHelper::Install (Ptr<Node> a, Ptr<Node> b)
{
  ...
  if (useNormalChannel)
    {
      channel = m_channelFactory.Create<QbbChannel> ();
      //std::cout << "===rhy test=== useNormalChannel Create<QbbChannel>\t\t" <<'\n';
    }
  else
    {
      channel = m_remoteChannelFactory.Create<QbbRemoteChannel> ();
  ...
  devA->Attach (channel);
  devB->Attach (channel);
  // std::cout << "===rhy test=== QbbHelper::Install()\t\t" <<channel->GetNDevices()<<'\n';
  // std::cout << "===rhy test=== QbbHelper::Install()\t\t" <<channel->GetDevice(0)<<'\n';
  // std::cout << "===rhy test=== QbbHelper::Install()\t\t" <<channel->GetDevice(1)<<'\n';
  // NS_ASSERT_MSG(channel->GetNDevices() == 2,"QbbHelper::Install(): channel not properly attached");
  container.Add (devA);
  container.Add (devB);
