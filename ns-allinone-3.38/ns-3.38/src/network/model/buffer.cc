...
Buffer::Allocate(uint32_t reqSize)
{
    ...
    uint8_t* b = new uint8_t[size*1];
    struct Buffer::Data* data = reinterpret_cast<struct Buffer::Data*>(b);
    // std::cout << "=== BUFFER ALLOCATION DIAGNOSTIC ===" << '\n';
    // std::cout << "Requested size: " << reqSize << " bytes" << '\n';
    // std::cout << "Actual allocation: " << size << " bytes" << '\n';
    // std::cout << "Buffer pointer: " << (void*)b << '\n';
    // std::cout << "Data pointer: " << (void*)data << '\n';   
    data->m_size = reqSize;
    data->m_count = 1;
    return data;
}
...
{
    //std::cout << "===rhy test=== 9999\t"<<'\n';
    NS_LOG_FUNCTION(this << start);
    NS_ASSERT(CheckInternalState());
    bool isDirty = m_data->m_count > 1 && m_start > m_data->m_dirtyStart;
    if (m_start >= start && !isDirty)
    {
        //std::cout << "===rhy test=== 9999-1\t"<<'\n';
