...
namespace ns3 {
	...
	TypeId BEgressQueue::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::BEgressQueue")
			...
			.AddTraceSource ("BeqEnqueue", "Enqueue a packet in the BEgressQueue. Multiple queue",
					MakeTraceSourceAccessor (&BEgressQueue::m_traceBeqEnqueue),"ns3::Packet::TracedCallBack")
			.AddTraceSource ("BeqDequeue", "Dequeue a packet in the BEgressQueue. Multiple queue",
					MakeTraceSourceAccessor (&BEgressQueue::m_traceBeqDequeue),"ns3::Packet::TracedCallBack")
			;
		...
		BEgressQueue::DoDequeueRR(bool paused[]) //this is for switch only
	{
		...
		if (m_queues[0]->GetNPackets() > 0u) //0 is the highest priority
		{
			found = true;
			qIndex = 0;
		}
		else
		{
			if (!found)
			{
				...
				{
					if (!paused[(qIndex + m_rrlast) % qCnt] && m_queues[(qIndex + m_rrlast) % qCnt]->GetNPackets() > 0u)  //round robin
					{
						found = true;
						break;
					}
