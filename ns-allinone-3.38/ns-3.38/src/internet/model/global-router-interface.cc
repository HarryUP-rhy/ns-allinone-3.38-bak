...
GlobalRouter::ProcessPointToPointLink(Ptr<NetDevice> ndLocal, GlobalRoutingLSA* pLSA)
{
    ...
    //Ptr<QbbNetDevice> nd_Local = DynamicCast<QbbNetDevice>(ndLocal);
    Ptr<Channel> ch = ndLocal->GetChannel();
    //Ptr<QbbChannel> ch = ndLocal->GetChannel();
    //rhy add  检查ndLocal是否真的在ch上
    // bool found = false;
    // for (uint32_t i = 0; i < ch->GetNDevices(); ++i) {
    //     if (ch->GetDevice(i) == ndLocal) {
    //         found = true;
    //         break;
    //     }
    // }
    // NS_ASSERT_MSG(found, "ndLocal is not attached to channel ch!");
...
GlobalRouter::GetAdjacent(Ptr<NetDevice> nd, Ptr<Channel> ch) const
{
    ...
    //std::cout << "===rhy test=== GlobalRouter::GetAdjacent\t\t" <<ch->GetNDevices()<<'\n';
    //std::cout << "===rhy test=== GlobalRouter::GetAdjacent\t"<<"nd"<<"\t" <<nd<<'\n';
    ...
    //
    Ptr<QbbChannel> channel = DynamicCast<QbbChannel>(ch);
    // Ptr<NetDevice> nd1 = ch->GetDevice(0);
    // Ptr<NetDevice> nd2 = ch->GetDevice(1);
    Ptr<NetDevice> nd1 = channel->GetDevice(0);
    Ptr<NetDevice> nd2 = channel->GetDevice(1);
    //Ptr<NetDevice> nd3 = channel->GetDevice(1);
    //std::cout << "===rhy test=== GlobalRouter::GetAdjacent\t"<<"nd1"<<"\t" <<nd1<<'\n';
    //std::cout << "===rhy test=== GlobalRouter::GetAdjacent\t"<<"nd2"<<"\t" <<nd2<<'\n';
    //std::cout << "===rhy test=== GlobalRouter::GetAdjacent\t"<<"nd3"<<"\t" <<nd3<<'\n';
    //
