...
NS_LOG_COMPONENT_DEFINE("TcpPacingExample");//便于理解or输出调试信息，not收集仿真数据

std::ofstream cwndStream;//实例化一个文件输出流类，记录cwnd的变化
std::ofstream pacingRateStream;
std::ofstream ssThreshStream;
std::ofstream packetTraceStream;//后续代码会将数据包的发送和接收时间、大小等信息写入到文件中
...
ConnectSocketTraces()
{
    Config::ConnectWithoutContext("/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow",//节点0上第一个tcp socket的拥塞控制窗口
