...
// Track RDMA traffic per node/port for debugging
std::map<uint32_t, std::map<uint32_t, uint64_t>> g_rdmaDeviceTxBytes;
std::map<uint32_t, std::map<uint32_t, uint64_t>> g_rdmaDeviceTxPkts;
...
void ReadFlowInput(){
	if (flow_input.idx < flow_num){
		...
		// std::cout << "===rhy test=== ReadFlowInput()\t"<<"n.GetN()"<<"\t" <<n.GetN()<<'\n';
...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		appCon.Start(Time(0));
		std::cout << "===rhy app install/start=== src=" << flow_input.src
          << " dst=" << flow_input.dst
          << " appNum=" << n.Get(flow_input.src)->GetNApplications()
          << std::endl;
		// 获取链路带宽
		uint64_t bw = pairBw[flow_input.src][flow_input.dst]; // 单位bps
		double flow_bits = flow_input.maxPacketCount * 8.0;
		double theory_time = flow_bits / bw; // 单位秒
		std::cout << "===rhy flow info=== idx=" << flow_input.idx
				<< " src=" << flow_input.src
				<< " dst=" << flow_input.dst
				<< " size=" << flow_input.maxPacketCount
				<< " bits=" << flow_bits
				<< " bw=" << bw
				<< " theory_time=" << theory_time << "s"
				<< " sim_time=" << simulator_stop_time << "s"
				<< std::endl;
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	std::cout << "===rhy test=== goto qp_finish\t"<<'\n';
	uint32_t sid = ip_to_node_id(q->sip), did = ip_to_node_id(q->dip);
	std::cout << "===rhy test=== goto qp_finish\t------1"<<'\n';
...
void TrackRdmaDeviceTx(uint32_t nodeId, uint32_t ifIndex, Ptr<const Packet> packet, Ptr<RdmaQueuePair> qp){
	if (packet == nullptr){
		return;
	}
	g_rdmaDeviceTxBytes[nodeId][ifIndex] += packet->GetSize();
	g_rdmaDeviceTxPkts[nodeId][ifIndex]++;
}
		...
		for (auto j = table.begin(); j != table.end(); j++){
			...
			for (int k = 0; k < (int)nexts.size(); k++){
				...
				if (node->GetNodeType() == 1)
					...
					if (!driver) {
						std::cerr << "ERROR: node " << node->GetId() << " has no RdmaDriver!" << std::endl;
						NS_ASSERT_MSG(driver, "RdmaDriver is nullptr!");
					}
					if (!driver->m_rdma) {
						std::cerr << "ERROR: node " << node->GetId() << " RdmaDriver->m_rdma is nullptr!" << std::endl;
						NS_ASSERT_MSG(driver->m_rdma, "m_rdma is nullptr!");
					}
...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (node_type[i] == 0){
			Ptr<Node> _node = CreateObject<Node>();
			//_node->GetNodeType(); // 确保关键节点类型为0
			//std::cout << "===rhy test=== main()\t"<<"NodeType"<<"\t" <<_node->GetNodeType()<<'\n';
			n.Add(_node);
	...
	for (uint32_t i = 0; i < link_num; i++)
	{
		...
		Ptr<QbbNetDevice> dev0 = DynamicCast<QbbNetDevice>(d.Get(0));
		Ptr<QbbNetDevice> dev1 = DynamicCast<QbbNetDevice>(d.Get(1));
		...
		nbr2if[snode][dnode].idx = dev0->GetIfIndex();
		nbr2if[snode][dnode].up = true;
		nbr2if[snode][dnode].delay = DynamicCast<QbbChannel>(dev0->GetChannel())->GetDelay().GetTimeStep();
		nbr2if[snode][dnode].bw = dev0->GetDataRate().GetBitRate();
		nbr2if[dnode][snode].idx = dev1->GetIfIndex();
		nbr2if[dnode][snode].up = true;
		nbr2if[dnode][snode].delay = DynamicCast<QbbChannel>(dev1->GetChannel())->GetDelay().GetTimeStep();
		nbr2if[dnode][snode].bw = dev1->GetDataRate().GetBitRate();
		...
		// setup PFC trace
		dev0->TraceConnectWithoutContext("QbbPfc", MakeBoundCallback (&get_pfc, pfc_file, dev0));
		dev1->TraceConnectWithoutContext("QbbPfc", MakeBoundCallback (&get_pfc, pfc_file, dev1));
		if (snode->GetNodeType() == 0){
			dev0->TraceConnectWithoutContext("RdmaQpDequeue", MakeBoundCallback(&TrackRdmaDeviceTx, snode->GetId(), dev0->GetIfIndex()));
		}
		if (dnode->GetNodeType() == 0){
			dev1->TraceConnectWithoutContext("RdmaQpDequeue", MakeBoundCallback(&TrackRdmaDeviceTx, dnode->GetId(), dev1->GetIfIndex()));
		}
	}
	...
	// // 新增文本跟踪文件
	// std::string rhy_test_file = "mix/rhy_test.txt";
	// std::string text_trace_file = rhy_test_file;
	// FILE *text_output = fopen(text_trace_file.c_str(), "w");
	// 	// dump link speed to text file in readable format
	// {
	// 	fprintf(text_output, "=== NS3 Simulation Settings ===\n");
	// 	fprintf(text_output, "Max BDP: %u\n", maxBdp);
	// 	fprintf(text_output, "\n=== Link Speeds ===\n");
	...
	// 			fprintf(text_output, "Node %d, Interface %d: %lu bps (%.2f Gbps)\n", 
	// 					node, intf, bps, bps / 1e9);
	// 		}
	// 	}
	// 	fprintf(text_output, "\n");
	// 	fflush(text_output);
	// 	fclose(text_output); // 关闭文本文件
	// }
	...
	// 新增：输出可读性更好的文本文件
	{
		// 创建文本文件名 - 将原文件的扩展名替换为.txt
		std::string rhy_test_file = "mix/rhy_test.txt";
		std::string text_trace_file = rhy_test_file;
		size_t dot_pos = text_trace_file.find_last_of('.');
		if (dot_pos != std::string::npos) {
			text_trace_file = text_trace_file.substr(0, dot_pos) + ".txt";
		} else {
			text_trace_file += ".txt";
		}
		...
		FILE *text_output = fopen(text_trace_file.c_str(), "w");
		if (text_output == NULL) {
			std::cerr << "Error: Cannot open text trace file " << text_trace_file << std::endl;
		} else {
			// 写入文件头部信息
			fprintf(text_output, "========================================\n");
			fprintf(text_output, "NS3 Simulation Configuration - Human Readable\n");
			fprintf(text_output, "Generated from: %s\n", trace_output_file.c_str());
			fprintf(text_output, "Simulation Time: %lu\n", (unsigned long)Simulator::Now().GetTimeStep());
			fprintf(text_output, "========================================\n\n");
			
			// 写入端口速度信息 - 与二进制文件完全相同的内容
			fprintf(text_output, "PORT SPEED CONFIGURATION:\n");
			fprintf(text_output, "=========================\n");
			...
			for (auto i: nbr2if){
				for (auto j : i.second){
					...
					// 格式化输出，与二进制文件内容一致
					fprintf(text_output, "Node: %-4d | Interface: %-3d | Speed: %-15lu bps | %-8.2f Gbps\n", 
							node, intf, bps, bps / 1e9);
					total_ports++;
				}
			}
			fprintf(text_output, "Total ports configured: %u\n\n", total_ports);
			
			// 写入窗口大小信息 - 与二进制文件完全相同的内容
			fprintf(text_output, "WINDOW CONFIGURATION:\n");
			fprintf(text_output, "=====================\n");
			fprintf(text_output, "Maximum Bandwidth-Delay Product (BDP): %u\n", maxBdp);
			...
			if (total_ports > 0) {
				...
				for (auto i: nbr2if){
					for (auto j : i.second){
						uint64_t bps = DynamicCast<QbbNetDevice>(i.first->GetDevice(j.second.idx))->GetDataRate().GetBitRate();
						total_speed += bps;
						if (bps < min_speed) min_speed = bps;
						if (bps > max_speed) max_speed = bps;
					}
				}
				
				uint64_t avg_speed = total_speed / total_ports;
				fprintf(text_output, "Average link speed: %lu bps (%.2f Gbps)\n", avg_speed, avg_speed / 1e9);
				fprintf(text_output, "Minimum link speed: %lu bps (%.2f Gbps)\n", min_speed, min_speed / 1e9);
				fprintf(text_output, "Maximum link speed: %lu bps (%.2f Gbps)\n", max_speed, max_speed / 1e9);
			}
			
			// 写入跟踪配置信息
			fprintf(text_output, "\nTRACE CONFIGURATION:\n");
			fprintf(text_output, "====================\n");
			fprintf(text_output, "Trace enabled: %s\n", enable_trace ? "YES" : "NO");
			
			// 如果有跟踪节点信息，输出它们
			if (enable_trace && trace_nodes.GetN() > 0) {
				fprintf(text_output, "Trace nodes (%u total): ", trace_nodes.GetN());
				for (uint32_t i = 0; i < trace_nodes.GetN(); ++i) {
					fprintf(text_output, "%d", trace_nodes.Get(i)->GetId());
					if (i < trace_nodes.GetN() - 1) {
						fprintf(text_output, ", ");
					}
				}
				fprintf(text_output, "\n");
			...
			fprintf(text_output, "\n========================================\n");
			fprintf(text_output, "End of Configuration File\n");
			fprintf(text_output, "========================================\n");
			
			fflush(text_output);
			fclose(text_output);
			
			std::cout << "Human-readable configuration saved to: " << text_trace_file << std::endl;
		}
	}
	...
	Simulator::Run();
	std::cout << "===rhy rdma-device-bytes===\n";
	if (g_rdmaDeviceTxBytes.empty()){
		std::cout << "no samples" << std::endl;
	}else {
		for (const auto &nodeStats : g_rdmaDeviceTxBytes){
			for (const auto &ifaceStats : nodeStats.second){
				uint64_t pktCnt = g_rdmaDeviceTxPkts[nodeStats.first][ifaceStats.first];
				std::cout << "node=" << nodeStats.first
					<< " if=" << ifaceStats.first
					<< " bytes=" << ifaceStats.second
					<< " pkts=" << pktCnt << std::endl;
			}
		}
	}
	Simulator::Destroy();
