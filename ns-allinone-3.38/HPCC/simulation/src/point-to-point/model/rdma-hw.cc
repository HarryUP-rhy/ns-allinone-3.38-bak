...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	const char *ackType = ch.l3Prot == 0xFD ? "NACK" : "ACK";
	int i;
	...
	if (m_ack_interval == 0)
		...
		uint64_t ackTarget = seq;
		if (!m_backto0){
			qp->Acknowledge(ackTarget);
		}else {
			uint32_t goback_seq = seq / m_chunk * m_chunk;
			ackTarget = goback_seq;
			qp->Acknowledge(goback_seq);
		}
		if (qp->snd_una != prevUna){
			std::cout << "[RDMA][" << ackType << "] node:" << m_node->GetId()
			          << " pg:" << qIndex << " sport:" << qp->sport
			          << " ack:" << ackTarget << " snd_una:" << qp->snd_una << "\n";
		}
		if (qp->IsFinished()){
			QpComplete(qp);
		}
...
void RdmaHw::QpComplete(Ptr<RdmaQueuePair> qp){
	...
	std::cout << "[RDMA][QpComplete] node:" << m_node->GetId()
	          << " pg:" << qp->m_pg << " flow:" << qp->sport << "->" << qp->dport
	          << " snd_una:" << qp->snd_una << " size:" << qp->m_size << "\n";
